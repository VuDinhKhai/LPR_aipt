...
def detect(weights_detect,weights_reco,video_path):
    ...
    # img0 = cv2.imread(r'E:\AIPT\yolov5\test_image\2.jpg')
    vid = cv2.VideoCapture(video_path) 
    frame_count = 0
    delay_frames = 8
    while(vid.isOpened()):
                ...
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det): # *xyxy : tọa độ bbox, conf : độ chính xác, cls : class mấy
                        ...
                        if conf > 0.7:
                            ...
                            crop = img0[y1:y2, x1:x2]
                            # cv2.imshow("crop" + str(n), crop)
                            value = detect_recognition_letter(weights_reco,crop)
                            print("bien so : " ,value)
                            # cv2.imwrite(r'E:\AIPT\yolov5\result\{}.jpg'.format(str(n)), crop)
                            n=n+1
...
if __name__ == '__main__':
    weights_detect=r'E:\AIPT\LPR_final\model\LP_detector_nano_61.pt'
    weights_reco = r'E:\AIPT\LPR_final\model\best_188.pt'
    video_path = r'E:\AIPT\LPR_final\test_image\5.mp4'
    with torch.no_grad():
